...
| 单帧检测耗时 | `detectForVideo` / `detect` 调用前后差值（图片模式为 5 次连续 detect 的中位数） | **≤ 40 ms** |
...
| 图片模式耗时上千 ms | 旧版单次测量口径混入了模式切换重建推理图、GPU 首次推理预热、手机原图全尺寸上传三笔一次性/失真成本。更新代码后重测（已加预热 + 长边 1280 缩放 + 5 次中位数），正常应与视频模式同量级（几十 ms）。 |
