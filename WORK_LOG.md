...
- ✅ 智谱 API Key 实测有效（认证通过；免费模型当时返回 1305 拥堵，稍后重试即可）。
- ⚠️→✅ 曾按用户要求内置 Key（`bea6ebe`），随后用户判断公开仓库内置不妥，**已撤回**（代码/测试/README 还原为「Key 仅存本机 localStorage」；保留 HTTP 200 错误体识别改进）。**注意：旧 Key 已进入公开 git 历史，需在智谱控制台吊销作废、另生成新 Key 自用**（新 Key 只填手机端设置页，不进仓库）。
