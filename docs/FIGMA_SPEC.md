...
> 迭代记录：2026-07-17 R1.1 点评页补回分享入口（照片右上角分享按钮，语义为调起 iOS 系统分享面板，不做社区）
...
| 玻璃圆钮 `btn-glass-circle` | 44×44 正圆，`effect/glass`，图标 21；用于取景器顶栏返回、翻转镜头、点评页分享按钮，并作 AR 开关基底（见 2.12）；按下 0.9 |
| 分享按钮 `btn-share`（R1.1 新增） | `btn-glass-circle` 基底（44×44，`effect/glass`），定位于点评照片右上 14/14；iOS 分享 glyph（方形 + 向上箭头）图标 21，stroke 2 圆头 `brand/cyan`；点击 Toast「已调起系统分享面板（演示）」，语义为调起 iOS 系统分享面板，不做社区 |
...
5. 分享按钮 `btn-share`：44×44 玻璃圆钮（`btn-glass-circle` 基底，规格见 2.14），叠在照片右上（右/上各 14，z 2 高于照片叠层）；iOS 分享 glyph（方形 + 向上箭头）图标 21×21（viewBox 24，stroke 2 圆头，`brand/cyan`）（R1.1 新增）
6. 反馈列表 `fb-list`：3 条反馈行（规格见 2.11），间距 10，下距 8：
   - 构图 ★★★★☆「人物落在右侧三分线，头顶留白舒适，构图很稳。」
   - 姿势 ★★★☆☆「右臂线条略僵，下次再放松一点会更自然。」
   - 情绪 ★★★★★「笑容自然有感染力，这张的情绪价值拉满。」
7. 页脚双按钮行（间距 12，各宽 165）：主按钮「再拍一张」（→ 屏 4）+ 次按钮「完成」（仅 Toast 反馈）；R1 起移除原主按钮「保存照片」

**交互：** 快门即存 —— 按下快门时照片已自动保存，saved-chip 作状态确认（无手动保存入口）；分享按钮 → Toast「已调起系统分享面板（演示）」（R1.1 补回分享入口，语义为调起 iOS 系统分享面板，不做社区）；「完成」→ Toast「拍摄完成，成片已在相册（演示）」；再拍一张 / 返回 → 屏 4。
...
- 图标与矢量图形（返回、刷新、关闭、翻转、对勾、AI 星标、反馈图标、品牌标、AR 轮廓、AR 开关人形、分享）：导出 **PDF 单倍矢量**（Xcode Asset Catalog 勾选 Preserve Vector Data），统一在各自 viewBox 画板（24 / 48 / 64）内导出；备选 SVG。
...
7. **点评页按钮组与「保存/分享」入口变更（R1；R1.1 已解决）**：7-17 规格「页面清单（5 屏）」第 5 条定义点评页含「重拍/保存按钮」，PRD 3.1 功能架构写拍摄后「一键保存/分享」；R1 原型改为「快门即存」——快门按下即自动保存，点评页评分 chip 同行新增 saved-chip 状态标记，页脚改为主按钮「再拍一张」+ 次按钮「完成」，移除手动「保存照片」按钮。**R1.1 解决**：分享入口已补回 —— 点评照片右上角新增分享按钮 `btn-share`，语义为调起 iOS 系统分享面板（原型以 Toast 演示），**不做社区**；保存维持「快门即存」、无手动入口。处理：以 R1.1 原型为准（规格见屏 5 与 2.14）。
